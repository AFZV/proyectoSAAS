...
export function LastOrders({ pedidos = [] }: LastOrdersProps) {
  ...
  const getNombreCliente = (cliente?: Cliente) => {
    ...
    if (cliente.nombre && cliente.apellidos) return `${cliente.nombre} ${cliente.apellidos}`;
  ...
  const pedidosParaMostrar = pedidos.length > 0 ? pedidos : [
    {
      id: "1234",
      total: 125000,
    ...
    <div className="bg-card border rounded-xl p-4">
      ...
      <div className="space-y-3">
        {pedidosParaMostrar.slice(0, 4).map((pedido, index) => {
          ...
          return (
            <div key={pedido.id} className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-lg transition-colors group">
              <div className="flex items-center space-x-2 flex-1 min-w-0">
                <div className={`w-6 h-6 bg-gradient-to-br ${avatarColors[index % avatarColors.length]} rounded-full flex items-center justify-center flex-shrink-0`}>
                  <span className="text-white text-xs font-semibold">
                    {getInitials(nombreCliente)}
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-foreground truncate">{nombreCliente}</p>
                  <div className="flex items-center space-x-1">
                    <span className="text-xs text-muted-foreground">#{pedido.id.slice(-4)}</span>
                    <span className={`inline-flex px-1.5 py-0.5 text-xs font-medium rounded-full ${getEstadoColor(estadoActual)}`}>
                      {estadoActual}
                    </span>
                  </div>
                </div>
              ...
              <div className="text-right flex-shrink-0">
                <p className="text-xs font-semibold text-foreground">{formatValue(pedido.total)}</p>
                <p className="text-xs text-muted-foreground">
                  {new Date(pedido.fechaPedido).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}
                </p>
              </div>
            </div>
          );
