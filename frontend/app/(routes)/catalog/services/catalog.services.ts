...
export class CatalogService {
  ...
  async getProductosParaCatalogo(token: string): Promise<Producto[]> {
    const [productosRes, categoriasRes] = await Promise.all([
      this.makeRequest<{ productos: ProductoBackend[] }>('/productos/empresa/activos', token),
      this.makeRequest<{ categorias: Categoria[] }>('/productos/categoria/empresa', token),
    ]);
  ...
  async getCategorias(token: string): Promise<Categoria[]> {
    const response = await this.makeRequest<{ categorias: Categoria[] }>('/productos/categoria/empresa', token);
  ...
  async buscarClientePorNitAlternativo(token: string, nit: string): Promise<Cliente> {
    try {
