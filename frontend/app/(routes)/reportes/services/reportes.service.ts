...
export class ReportesService {
    ...
    static async generarInventarioGeneral(data: {
        ...
        const response = await fetch(`${BACKEND_URL}/reportes/inventario/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
        ...
        await this.handleDownload(response, `reporte-inventario.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarInventarioRango(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/inventario/rango/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
        ...
        await this.handleDownload(
            response,
            `inventario_${data.inicio}-${data.fin}.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`
        );
    }
     ...
     */
    static async generarClientesTodas(data: {
        formato: 'excel' | 'pdf';
    }) {
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/clientes/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({}),
        });

        await this.handleDownload(response, `reporte-clientes.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarClientesCiudad(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/clientes/ciudad/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                ciudad: data.ciudad,
        ...
        await this.handleDownload(response, `clientes-${data.ciudad}.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarClientesVendedor(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/clientes/vendedor/${data.vendedorId}/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({}),
        });

        await this.handleDownload(response, `clientes-vendedor.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarPedidosTodos(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/pedidos/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                fechaInicio: new Date(data.fechaInicio).toISOString(),
        ...
        await this.handleDownload(response, `reporte-pedidos.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarPedidosVendedor(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/pedidos/${data.formato}/${data.vendedorId}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                fechaInicio: new Date(data.fechaInicio).toISOString(),
        ...
        await this.handleDownload(response, `pedidos-vendedor.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarCarteraGeneral(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/cartera/${data.formato}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                fechaInicio: new Date(data.fechaInicio).toISOString(),
        ...
        await this.handleDownload(response, `cartera-pendiente.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
    ...
    static async generarCarteraVendedor(data: {
        ...
        const headers = await this.getHeaders();
        const response = await fetch(`${BACKEND_URL}/reportes/cartera/${data.formato}/vendedor/${data.vendedorId}`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                fechaInicio: new Date(data.fechaInicio).toISOString(),
        ...
        await this.handleDownload(response, `cartera-vendedor.${data.formato === 'excel' ? 'xlsx' : 'pdf'}`);
    }
